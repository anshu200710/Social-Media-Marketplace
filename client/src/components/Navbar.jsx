...
import { BoxIcon, CrossIcon, GrabIcon, GripIcon, ListIcon, MenuIcon, MessageSquareMoreIcon, XIcon } from 'lucide-react'
  ...
  return (
    ...
    <nav className='h-20'>
                <div className='fixed left-0 top-0 right-0 z-100 flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white transition-all'>
                    <img src={assets.logo} onClick={()=> {navigate('/'); scrollTo(0,0)}} alt="" className='h-10 cursor-pointer' />
                    ...
                    <div className='hidden sm:flex items-center gap-4 md:gap-8 max-md:text-sm text-gray-800'>
                        ...
                        <Link to={user ? '/message' : ''} onClick={() => user ? scrollTo(0, 0): openSignIn()}> Messages </Link>
                     ...
                     { !user ? (
                        ...
                        <UserButton>
                            ...
                            <UserButton.MenuItems>
                                <UserButton.Action label='Messages' labelIcon={<MessageSquareMoreIcon size={16} />} onClick={() => navigate('/messages')} />
                            </UserButton.MenuItems>
                ...
                <div className={`sm:hidden fixed inset-0 ${menuOpen ? 'w-full' : 'w-0'} overflow-hidden bg-white backdrop-blur shadow-xl rounded-lg z-[200] text-sm transition-all`}>
                    <div className='flex flex-col items-center justify-center h-full text-xl font-semibold gap-6 p-4'>
                        ...
                        <XIcon onClick={() => setMenuOpen(false)} className='absolute size-8 right-6 top-6 text-gray-600 hover:text-gray-700 cursor-pointer'/>
                    </div>
                </div>
            </nav>
    </>
